	...
	for (i = 0; list[i].attachfn; i++) {
		if (list[i].key1 == key1 && list[i].key2 == key2) {
			...
			if (r > 0)
				return r;
			if (r < 0)
				printk("pci_attach_match: attaching "
					"%x.%x (%p): e\n",
...
pci_print_func(struct pci_func *f)
{
	...
	printk("PCI: %02x:%02x.%d: %04x:%04x: class: %x.%x (%s) irq: %d\n",
...
pci_bridge_attach(struct pci_func *pcif)
{
	...
	if (PCI_BRIDGE_IO_32BITS(ioreg)) {
		printk("PCI: %02x:%02x.%d: 32-bit bridge IO not supported.\n",
	...
	if (pci_show_devs)
		printk("PCI: %02x:%02x.%d: bridge to PCI bus %d--%d\n",
			pcif->bus->busno, pcif->dev, pcif->func,
			nbus.busno,
...
pci_func_enable(struct pci_func *f)
{
	     ...
	     bar += bar_width)
	{
		...
		if (PCI_MAPREG_TYPE(rv) == PCI_MAPREG_TYPE_MEM) {
			...
			if (pci_show_addrs)
				printk("  mem region %d: %d bytes at 0x%x\n",
					regnum, size, base);
		} else {
		...
		if (size && !base)
			printk("PCI device %02x:%02x.%d (%04x:%04x) "
	...
	printk("PCI function %02x:%02x.%d (%04x:%04x) enabled\n",
		f->bus->busno, f->dev, f->func,
		PCI_VENDOR(f->dev_id), PCI_PRODUCT(f->dev_id));
}
